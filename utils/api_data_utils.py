import requests
import time
from ..config  import (DB2_HISTORICAL_TABLE, BASE_URL, ACCESS_KEY)
...
def _fetch_currency_data_for_month(year, month) -> dict:
    ...
    for day in range(1,31):
        month_data[day] = _fetch_currency_date(year,month,day)
    return month_data
...
def _fetch_currency_data_for_year(year) -> dict:
    ...
    for month in range(1,13):
        year_data[month] = _fetch_currency_date(year, month, 1)
    return year_data

# --- Fetch currency data for a time series (30 consecutive days at most)
def fetch_currency_data_for_time_series(year: int, month: int, start_day: int, end_day: int) -> dict:
    time_series_data = {}
