from .utils.api_data_utils import (fetch_currency_data_for_time_series, _fetch_currency_data_for_year, _fetch_currency_data_for_month)

# --- Fetch currency data for a month
def _get_currency_data_for_month(year, month) -> dict:
    return _fetch_currency_data_for_month(year, month)
# --- Fetch currency data for a year (01-MM for a year)
def _get_currency_data_for_year(year) -> dict:
    return _fetch_currency_data_for_year(year)
...
def _get_currency_data_for_time_series(year: int, month: int, start_day: int, end_day: int) -> dict:
    return _fetch_currency_data_for_time_series(year,month,start_day,end_day)
